def test_create_new_income_categorie(test_client, init_database, insert_user_db):
...
def test_create_new_expense_categorie(test_client, init_database, insert_user_db):
    login_url = '/api/v1/auth/login'
    login_data = {
        'email': insert_user_db.email,
        'password': 'password'
...
def test_error_create_category_with_invalid_type(test_client, init_database, insert_user_db):
    login_url = '/api/v1/auth/login'
    login_data = {
        'email': insert_user_db.email,
        'password': 'password'
...
def test_error_create_new_categorie_with_inexistent_user(test_client, init_database, insert_user_db):
    login_url = '/api/v1/auth/login'
    login_data = {
        'email': insert_user_db.email,
        'password': 'password'
...
def test_list_user_without_categories(test_client, init_database, insert_user_db):
    login_url = '/api/v1/auth/login'
    login_data = {
        'email': insert_user_db.email,
        'password': 'password'
...
def test_list_user_with_categories(test_client, init_database, insert_user_db, insert_category_db):
    ...
    assert response.json[0]['name'] == 'UnitTest'
    assert response.json[0]['type'] == 'Entrada'