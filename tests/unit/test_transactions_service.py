...
def test_income_transactions_service_method(mock_get_sqlalchemy, mock_account_object, mock_e_category_object,
                                            mocker):
...
def test_expense_error_category_not_fount(mock_get_sqlalchemy):
    mock_get_sqlalchemy.filter_by.return_value.first.return_value = None
    with pytest.raises(CategoryNotFound):
        TransactionsService.expense('user_id', 'account_id', {'value': 1, 'category': 'Category'})

def test_expense_error_account_not_found(mock_get_sqlalchemy, mock_s_category_object):
    mock_get_sqlalchemy.filter_by.return_value.first.side_effect = [mock_s_category_object, None]
    with pytest.raises(AccountNotFound):
        TransactionsService.expense('user_id', 'account_id', {'value': 1, 'category': 'Category'})

def test_expense_error_account_incorrect_category(mock_get_sqlalchemy, mock_e_category_object, mock_user_object):
    mock_get_sqlalchemy.filter_by.return_value.first.side_effect = [mock_e_category_object, mock_user_object]
    with pytest.raises(IncorrectCategory):
        TransactionsService.expense('user_id', 'account_id', {'value': 1, 'category': 'Category'})

def test_expense_transactions_service_method(mock_get_sqlalchemy, mock_account_object, mock_s_category_object,
                                            mocker):
...
def test_extract_error_account_not_found(mock_get_sqlalchemy):
    mock_get_sqlalchemy.filter_by.return_value.first.return_value = None
    with pytest.raises(AccountNotFound):
        TransactionsService.extract('user_id', 'account_id')

def test_extract_transactions_service_method(mock_get_sqlalchemy, mock_account_object, mock_transaction_object,
                                             mocker):
